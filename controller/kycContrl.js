...
// // ------------------ HELPERS ------------------
// const hashIdentityNumber = (idNumber, salt) => {
//   return crypto.pbkdf2Sync(idNumber, salt, 1000, 64, "sha512").toString("hex");
// };
...
//           email:
//             data.email ||
//             `${data.first_name}.${data.last_name}@texttopay.com`, // fallback
//           provider: "mono",
//         };
//       }
...
//     const bvnHash = hashIdentityNumber(bvn, salt);
    
//     // Check all KYC records to see if this BVN already exists
//     const allKYCRecords = await KycverifModel.find({});
    
//     for (const record of allKYCRecords) {
//       const testHash = hashIdentityNumber(bvn, record.salt);
//       if (testHash === record.idNumberHash) {
...
//     // ---- 🔐 Securely hash identity number ----
//     const salt = crypto.randomBytes(16).toString("hex");
//     const idNumberHash = hashIdentityNumber(identityNumber, salt);

//     // ---- Save in KYC collection (ONLY hashed data, NO plain BVN) ----
//     const kycRecord = await KycverifModel.findOneAndUpdate(
//       { userid: userId },
//       {
//         userid: userId,
//         idType: identityType, // 'bvn', 'nin', etc.
//         idNumberHash, // Only store hashed version
//         salt,
...
//     // ---- Update User collection with verified info (NO BVN stored here) ----
